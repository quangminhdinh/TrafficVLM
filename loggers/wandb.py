...
class WandbLogger(ExperimentLogger):
    ...
    @classmethod
    def from_xp(cls, log_folder: str, signature: str, 
                with_media_logging: bool = True, project: tp.Optional[str] = None, 
                name: tp.Optional[str] = None, group: tp.Optional[str] = None, 
                **kwargs: tp.Any):
        assert project is not None
        xp_folder = Path(log_folder) / project
        save_dir = xp_folder
        save_dir.mkdir(exist_ok=True)
        flag_file = xp_folder / 'wandb_flag'
        ...
        if wandb:
            ...
            try:
                if project:
                    run = api.run(project + '/' + signature)
                else:
                    run = api.run(signature)
                ...
                name = run.name
                config = run.config
        return WandbLogger(str(save_dir), with_media_logging, id=signature, name=name or signature, group=group,
